...
@implementation SetMessageCommand
...
-(void)initializeFields
{
    ...
    for(Field *field in self.commandFields.fields)
    {
        ...
        else if([field.fieldname isEqualToString:@"Slot Number:"])
        {
            self.slotNo = (int) field.value;
        }
...
//    commandLen = ERASE_MESSAGE_COMMAND_SIZE;
    ...
    {
        buffer[i+2] = [SetMessageCommand getIntValue:data startIndex:start length:2];
        start+=2;
    }
...
-(NSMutableArray*)getCommandData
{
    ...
    temp = [temp stringByAppendingString:[NSString stringWithFormat: @"%.4X",(unsigned short)self.messageCode]];
    temp = [temp stringByAppendingString:[NSString stringWithFormat: @"%.2X",(unsigned short)self.slotNo]];
